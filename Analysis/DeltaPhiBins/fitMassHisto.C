...
void fitMassHisto(TString inputfile="outfiles/MassHisto", TString outputfile="outfiles/DeltaPhiHisto",
                  Float_t centmin=0., Float_t centmax=100.)
{
  ...
  TF1* fit(Float_t ptmin, Float_t ptmax, Int_t j);
  ...
  for(int i=0;i<nPtBins;i++)
    {
      ...
      for(int j=0;j<nPhiBins;j++)
        {
          TF1* f = fit(ptBins[i],ptBins[i+1],j);
          ...
          hPhi->SetBinContent(j+1,yield/(dphistep*(ptBins[i+1]-ptBins[i])));
          hPhi->SetBinError(j+1,yieldErr/(dphistep*(ptBins[i+1]-ptBins[i])));
        }
...
TF1* fit(Float_t ptmin, Float_t ptmax, Int_t j)
{
  ...
  TFile* infile = new TFile(Form("%s_cent_%.0f_%.0f_pt_%.0f_%.0f_dphi_%d.root",infname.Data(),centMin,centMax,ptmin,ptmax,j));
  TH1F* h = (TH1F*)infile->Get("h");                    h->SetName(Form("h_%.0f_%.0f_%d",ptmin,ptmax,j));
  TH1F* hMCSignal = (TH1F*)infile->Get("hMCSignal");    hMCSignal->SetName(Form("hMCSignal_%.0f_%.0f_%d",ptmin,ptmax,j));
  TH1F* hMCSwapped = (TH1F*)infile->Get("hMCSwapped");  hMCSwapped->SetName(Form("hMCSwapped_%.0f_%.0f_%d",ptmin,ptmax,j));
  ...
  c->SaveAs(Form("plots/Mass_cent_%.0f_%.0f_pt_%.0f_%.0f_dphi_%d.pdf",centMin,centMax,ptmin,ptmax,j));
...
int main(int argc, char *argv[])
{
  if(argc==5)
    {
      fitMassHisto(argv[1],argv[2], atof(argv[3]), atof(argv[4]));
      return 0;
    }
  else
    {
