  ...
  for(int i=0;i<nPtBins;i++)
    {
      ...
      for(int j=0;j<nPhiBins;j++)
        {
          ah[i][j] = new TH1D(Form("h_%d_%d",i,j),"",nbinsmasshisto,minhisto,maxhisto);          
        }
    }
  ...
  for(int i=0;i<nentries;i++)
    {
      ...
      for(int j=0;j<Dsize;j++)
        {
          Int_t* histno = new Int_t[2];
          findhistno(j,true,histno);
          if(histno[0]<0 || histno[1]<0) continue;
          if(!passcutsel(histno[0],j)) continue;
          ah[histno[0]][histno[1]]->Fill(Dmass[j]);
        }
    }
  ...
  for(int i=0;i<nentries;i++)
    {
      ...
      ntMCSkim->GetEntry(i);
      if(i%1000000==0) cout<<setiosflags(ios::left)<<"    "<<setw(8)<<i<<" / "<<nentries<<endl;
      if(!passeventsel(centmin,centmax)) continue;
      for(int j=0;j<Dsize;j++)
        {
          Int_t* histno = new Int_t[2];
          findhistno(j,false,histno);
          if(histno[0]<0) continue;
          if(!passcutsel(histno[0],j)) continue;
          if(Dgen[j]==23333) ahMCSignal[histno[0]]->Fill(Dmass[j]);
          if(Dgen[j]==23344) ahMCSwapped[histno[0]]->Fill(Dmass[j]);
        }
    }
  ...
  for(int i=0;i<nPtBins;i++)
    {
      for(int j=0;j<nPhiBins;j++)
        {
          TH1D* h = (TH1D*)ah[i][j]->Clone("h");
          TH1D* hMCSignal = (TH1D*)ahMCSignal[i]->Clone("hMCSignal");
          TH1D* hMCSwapped = (TH1D*)ahMCSwapped[i]->Clone("hMCSwapped");
          TFile* outf = new TFile(Form("%s_cent_%.0f_%.0f_pt_%.0f_%.0f_dphi_%d.root",outputfile.Data(),centmin,centmax,ptBins[i],ptBins[i+1],j),"recreate");
          outf->cd();
          h->Write();
          hMCSignal->Write();
          hMCSwapped->Write();
